...
class PlaneGUI:
    ...
    def initialize_gui(self):
        ...
        self.left_frame = tkinter.Frame(self.root).pack(side="left")
        self.right_frame = tkinter.Frame(self.root).pack(side="right")

        self.canvas = tkinter.Canvas(self.left_frame)
        self.canvas.pack(side="left", fill="both", anchor='nw')#, expand=True)

        self.panel = tkinter.Frame(self.right_frame)
...
#                or (x,y) in list(self.polygon.exterior.coords):
    ...
    def calculate_p(self):
        for p_0 in self.polygon_grid_points:
            found = False
        ...
        p_score = p_score / len(self.fundamental_circles)
...
#polygon = Polygon([[0,0], [0,2], [1,2], [1,0]])
#translation_vectors = np.array([(1,0), (0,2)])
#
#polygon = Polygon([[0,0], [1,0], [1.5,1], [0.5,1]])
#translation_vectors = np.array([(1,0), (0.5,1)])
#
#polygon = Polygon([
#    [0,0],
#    [0.5, math.sqrt(3)/2],
...
#    [0.5,-math.sqrt(3)/2]
#])
#translation_vectors = np.array([(1.5, math.sqrt(3)/2), (1.5, -math.sqrt(3)/2)])
#
#polygon = Polygon([[0,0], [1.2,0.3], [2.0,1.3], [0.8,1.0]])
#translation_vectors = np.array([[1.2,0.3],[0.8,1.0]])

# circles = [Point(0,0).buffer(1.45)]
